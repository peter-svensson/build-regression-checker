...
public class AnalysisRegressionCheckerTest extends HudsonTestCase {
	...
	public void testBuildFailsWhenCoverageDecreases() throws Exception {
		...
		FilePath coberturaXml = p.getLastBuild().getWorkspace()
				.child("coverage.xml");

		p.getPublishersList().add(
				new CoberturaPublisher("coverage.xml", true, SourceEncoding
						.getEncoding("UTF-8")));
		...
		p.getPublishersList().add(
				new AnalysisRegressionChecker(true, false, true, true));
